...
* 🎮 **Twitch連携ツール** (`TwitchPlugin.py`)  
  Twitchチャンネル名またはURLを指定するだけで、視聴者のチャットコメントをリアルタイムに拾ってAIに反応させることができます。OAuth2認証を行うと、コメント書き込み・アンケート作成・予測（ベッティング）・クリップ作成・配信タイトル変更・視聴者数取得などの操作もAIが自動で実行できます。**認証なし（IRCモード）でもチャット読み取りは動作します。**  
