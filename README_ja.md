...
* 📺 **YouTube Live** (`YoutubeLivePlugin.py`)  
  YouTube Liveの配信URLを指定するだけで、リスナーのコメントを拾ってAIに反応させることができます。配信の「タイトル」「説明文」「サムネイル画像」も自動でAIに送り込まれます。  
  👉 [詳細な使い方はこちら](docs/ja/plugins/YoutubeLivePlugin.md)

* 📺 **Twitch Live** (`TwitchPlugin.py`)  
  Twitchチャンネル名またはURLを指定するだけで、視聴者のチャットコメントをリアルタイムに取得。OAuth2認証でコメント書き込み・アンケート・予測・クリップ・タイトル変更なども可能。**認証なし（IRCモード）でもチャット読み取りは動作します。**  
  👉 [詳細な使い方はこちら](docs/ja/plugins/TwitchPlugin.md)

* 📺 **ニコニコ生放送** (`NiconicoLivePlugin.py`)  
  ニコニコ生放送のコメント・ギフト・ニコニ広告・アンケート・統計情報をリアルタイムに取得。ログインでアンケート作成・運営コメント投稿も可能。**ログインなしでもコメント読み取りは動作します。**  
  👉 [詳細な使い方はこちら](docs/ja/plugins/NiconicoLivePlugin.md)

* 🎮 **OBS画面キャプチャ** (`obs_capture.py`)  
  OBS-WebSocketでプレビュー画面を定期キャプチャし、AIにゲーム画面や配信画面について実況・反応させます。  
  👉 [詳細な使い方はこちら](docs/ja/plugins/obs_capture.md)

* 💉 **カンペ・画像注入** (`ManualInjector.py`)  
  配信中に専用の操作パネルから、ボタン1つでカンペや画像をAIに直接送り込みます。  
  👉 [詳細な使い方はこちら](docs/ja/plugins/ManualInjector.md)

* 📝 **カスタム指示追加** (`custom_prompt.py`)  
  配信開始時にAIの性格プロンプトへ追加指示（例：「今日は〇〇のゲームをプレイ」）を付与します。  
  👉 [詳細な使い方はこちら](docs/ja/plugins/custom_prompt.md)

* 💬 **コメジェネ読み込み** (`CommentGenerator_read.py`)  
  外部のコメントジェネレーターが出力するテキストファイルを定期読み込みし、新しいコメントをAIに伝えます。  
  👉 [詳細な使い方はこちら](docs/ja/plugins/CommentGenerator_read.md)

* 📺 **テロップビューア** (`TelopViewer.py`)  
  AIが出力したテロップ履歴をリアルタイムに確認できる操作パネル。B1/B2タグの色分け表示対応。

### 🌟 公式拡張プラグイン（個別ダウンロード）

本体を軽くシンプルに保つため、以下の拡張機能は必要な方だけが個別にダウンロードして追加する形式になっています。  
拡張プラグインは専用リポジトリで管理されています。TeloPon内の **プラグイン管理** からもダウンロードできます。

🔗 **[TeloPon Extensions（拡張プラグイン一覧）](https://github.com/miyumiyu/TeloPon-Extensions)**

| プラグイン | 説明 |
|---|---|
| 📺 **YouTube Live+** | YouTube OAuth連携。コメント読み書き・アンケート・タイトル変更・視聴者数取得 |
| 🐦 **X (Twitter)** | ハッシュタグ取得・テキスト/サムネ/画面キャプチャ付きツイート投稿（有料API） |
| 🔊 **テロップ読み上げ (Windows)** | Windows標準音声（SAPI5）でテロップを自動読み上げ |
...
| 🎮 **VCI テロップ送信** | VirtualCast VCI へOSCでテロップ送信 |

*(💡 導入方法: 上記リンクからダウンロードした `.py` ファイルを `plugins` フォルダに入れるだけ、またはプラグイン管理からダウンロード！)*
