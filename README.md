...
> **⚠️ ダウンロードされる方へ重要なお知らせ**  
> このリポジトリは「TeloPon 早期アクセス版（実行ファイル）」の**配布専用ページ**です。ソースコードの公開・開発用リポジトリではありません。
> 
> **ダウンロードは、必ず画面右側にある [Releases] ページから行ってください。**  
...
* 📝 **カスタム指示追加ツール** (`custom_prompt.py`)  
  ...
  *(※プロンプト自体の書き換えや、配信途中での動的な指示変更はできません)*  
...
* **⏳ 最初の挨拶を準備中...**: 接続直後。AIが「最初の挨拶」を考えています。
...
* **⚠️ 割り込み検出 (Barge-in)** (赤色): AIが喋っている最中にあなたが話し始めたため、AIが空気を読んで自分の発言を途中でキャンセルした状態です。
...
4. **AI生成コードの性質**: 本ツールはAIとの対話によって開発された部分を含んでおり、予期せぬ挙動が発生する可能性があります。
