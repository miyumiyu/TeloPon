...
## 🏗️ 2. 【重要】HTMLの構造と３つのテロップ枠

TeloPonの画面（`base.html`）には、大きく分けて **「通常枠（normal）」**、**「特別枠（special）」**、**「ステータス枠（status）」** の3つのテロップコンテナが最初から用意されています。
AIが指定した「Window ID（後述）」によって、どの枠が使われるかが自動的に決まります。
...
### ③ ステータス枠（statusグループ） — 永続表示
TRPGのHP/SAN表示・初見さんカウンター・配信時間など、**明示的に消すまで画面に出続ける** 永続ウィンドウです。1ページに1つ固定で、再送信すると同じ枠の内容が**上書き更新**されます。

```html
<!-- ステータス枠の入れ物（ページに1つ固定） -->
<div id="status-container">

    <!-- ステータス枠の左上バッジ（デフォルト非表示） -->
    <div id="st-badge"></div>

    <!-- テキスト本体 -->
    <div id="st-text-wrapper">
        <div class="topic-line" id="st-topic">タイトル（上段）</div>
        <div class="main-line"  id="st-main">本文（下段）</div>
    </div>
...
**ステータス枠の特徴:**
- `--display-duration: 0` を指定することで「自動消去なし」になります
- 配信者は **OBSのブラウザソース上で直接操作可能**: ドラッグで移動 / ホイールで拡大縮小 / ダブルクリックで一時非表示
- 削除コマンド: AIが `[CMD]SYS:close status` を発行する、または「ステータス消して」と配信者が言う
...
### ステータス枠（statusグループ）のセレクター

| セレクター | 種別 | 役割 |
|---|---|---|
| `#status-container` | ID | ステータス枠全体の入れ物。Window ID のクラスがここに付く |
| `#status-container.visible` | ID＋クラス | ステータス枠が表示中の状態 |
| `#status-container.dragging` | ID＋クラス | 配信者がドラッグ中の状態（カーソル表示変更等に） |
| `#st-badge` | ID | ステータス枠の左上バッジ（デフォルトは `display: none !important;`） |
| `#st-text-wrapper` | ID | ステータス枠のテキストラッパー |
| `#st-topic` | ID | ステータス枠のトピック行（タイトル） |
| `#st-main` | ID | ステータス枠のメインテキスト行（本文） |
...
.window-status {
    ...
    --window-group: special;     /* ※ 互換用に special を指定（実グループは "status" に固定） */
    --display-duration: 0;        /* ★ 0 を指定すると永続表示（自動消去されない） */
}
...
> **重要**: `window-status` のグループは Python 側で `"status"` に固定されています（CSSの `--window-group` 指定値に関わらず）。HP/MP/カウンター等の常時表示用として独立した1枠が用意されており、通常枠や特別枠とは独立に表示されます。
...
### 📌 ステータス枠（statusグループ）のID
これらは `#status-container` に適用されます。**`--display-duration: 0`** を指定することで永続表示になります（明示的に消すまで残る）。

* **`.window-status`** : HP/SAN/カウンター/配信時間など、**常時表示したい情報**用。

| 用途 | 例 |
|---|---|
| TRPGステータス | 探索者の名前・職業・HP・SAN等を常時表示 |
| 初見さんカウンター | 「初見さん N人」を常時表示 |
| 配信時間 | 「2時間15分経過」など |

**操作:**
- 表示更新: 同じ `window-status` で再送信すると **上書き更新**（同じ枠の内容が新しくなる）
- 削除: AI が `[CMD]SYS:close status` を発行 / 配信者が「ステータス消して」と言う
- OBS上での操作: ドラッグで移動 / マウスホイールで拡大縮小 / ダブルクリックで一時非表示
