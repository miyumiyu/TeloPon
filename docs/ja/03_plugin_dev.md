...
|---|---|---|
| `is_connected` 属性あり | `is_connected = True` のとき | YouTube・Twitch など外部サービス接続型 |
| `is_running` 属性あり（`is_connected` なし） | `is_running = True` のとき | バックグラウンド処理型 |
| どちらも属性なし | `enabled = True` のとき | シンプルなカスタムプラグイン |
