...
| **CMD 명령 플러그인** | AI의 출력에서 `[CMD:XXX]`라고 말할 때 호출되는 처리를 담당합니다. | `plugin_loader.py`에 한 줄 추가 필요 |
...
## 4. AI에 정보 주입하기 (UI 패널 플러그인의 주요 역할)
...
class AutoTimerPlugin(BasePlugin):
    ...
    def __init__(self):
        ...
        self.is_connected = False  # UI와 동기화된 연결 상태 플래그
        self.thread       = None
...
class ManualCuePlugin(BasePlugin):
    ...
    def get_default_settings(self):
        return {"enabled": True}  # TOOL 타입은 기본적으로 항상 True
...
### plugin_loader.py에 등록

CMD 플러그인은 **`plugin_loader.py`에 수동으로 추가해야 합니다**.
(UI 패널 플러그인과 달리 자동 스캔되지 않습니다.)
...
from plugins.my_command_plugin import MyCommandPlugin  # ← 이것을 추가
...
def load_plugins(ui):
    plugins = [
        ...
        MyCommandPlugin(),  # ← 이것을 추가
    ]

    for plugin in plugins:
        cmd_dispatcher.dispatcher.register_plugin(plugin)

    logger.info(f"[PluginLoader] {len(plugins)}개 플러그인 등록됨")
```
...
하이브리드 플러그인도 `plugin_loader.py`에 항목을 추가해야 합니다 (CMD 등록을 위해).

---

## 9. 기존 플러그인 예시: img_plugin.py
...
- [ ] `plugin_loader.py`에 임포트 및 등록 추가
...
**CMD가 반응하지 않음**
→ `plugin_loader.py`에 등록이 추가되었는지 확인하세요. UI 플러그인과 달리 자동 스캔되지 않습니다.
...
**`[CMD:IMG]` 설정이 적용되지 않음**
→ `img_plugin.py`가 `plugins/` 폴더에 없으면 UI에 표시되지 않고 `plugin_loader.py` 등록도 실패합니다. 파일이 존재하는지 확인하세요.
