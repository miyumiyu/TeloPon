...
|---|---|---|
| `is_connected` 속성 있음 | `is_connected = True` 일 때 | YouTube·Twitch 등 외부 서비스 연결형 |
| `is_running` 속성 있음 (`is_connected` 없음) | `is_running = True` 일 때 | 백그라운드 처리형 |
| 둘 다 속성 없음 | `enabled = True` 일 때 | 단순한 커스텀 플러그인 |
...
→ `__init__`에서 `self.is_connected = False` 또는 `self.is_running = False`를 정의하면 `enabled`보다 이것이 우선됩니다. 외부 서비스 연결이 필요 없는 플러그인은 `is_connected` / `is_running`을 정의하지 마세요.
