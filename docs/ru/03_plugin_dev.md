...
### Поведение по паттернам

| Реализация плагина | Условие активации значка | Типичное использование |
|---|---|---|
| Атрибут `is_connected` есть | Когда `is_connected = True` | YouTube·Twitch и другие подключения к внешним сервисам |
| Атрибут `is_running` есть (без `is_connected`) | Когда `is_running = True` | Фоновая обработка |
...
## 6. Полный шаблон плагина BACKGROUND
...
## 8. Как создать плагин команд CMD
...
## 9. Гибридный плагин: объединение панели интерфейса и CMD
...
## 10. Пример существующего плагина: img_plugin.py
...
## 12. Контрольный список разработки плагинов
...
→ Если в `__init__` определены `self.is_connected = False` или `self.is_running = False`, они имеют приоритет над `enabled`. Плагины, не требующие подключения к внешним сервисам, не должны определять `is_connected` / `is_running`.
