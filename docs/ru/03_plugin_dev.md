...
| **Плагин команд CMD** | Обрабатывает процессы, вызываемые когда ИИ пишет `[CMD:XXX]` в своём выводе. | Требует добавления одной строки в `plugin_loader.py` |
...
## 3. Как создать плагин панели интерфейса
...
## 4. Вставка информации в ИИ (основная роль плагинов панели интерфейса)
...
## 5. Полный шаблон плагина BACKGROUND
...
class AutoTimerPlugin(BasePlugin):
    ...
    def __init__(self):
        ...
        self.is_connected = False  # Флаг состояния соединения, синхронизированный с интерфейсом
        self.thread       = None
...
class ManualCuePlugin(BasePlugin):
    ...
    def get_default_settings(self):
        return {"enabled": True}  # Тип TOOL по умолчанию всегда True
...
## 7. Как создать плагин команд CMD
...
### Регистрация в plugin_loader.py

CMD плагины **необходимо вручную добавить в `plugin_loader.py`**.
(В отличие от плагинов панели интерфейса, они не сканируются автоматически.)
...
from plugins.my_command_plugin import MyCommandPlugin  # ← Добавить это
...
def load_plugins(ui):
    plugins = [
        ...
        MyCommandPlugin(),  # ← Добавить это
    ]

    for plugin in plugins:
        cmd_dispatcher.dispatcher.register_plugin(plugin)

    logger.info(f"[PluginLoader] Зарегистрировано {len(plugins)} плагинов")
```
...
## 8. Гибридный плагин: объединение панели интерфейса и CMD
...
Гибридные плагины также требуют добавления записи в `plugin_loader.py` (для регистрации CMD).

---

## 9. Пример существующего плагина: img_plugin.py
...
## 10. Сводка часто используемых методов и утилит
...
## 11. Контрольный список разработки плагинов
...
- [ ] Добавлен импорт и регистрация в `plugin_loader.py`
...
## 12. Распространённые ошибки и способы их исправления
...
**CMD не реагирует**
→ Убедитесь, что регистрация добавлена в `plugin_loader.py`. В отличие от плагинов интерфейса, автоматическое сканирование не выполняется.
...
→ Если `img_plugin.py` отсутствует в папке `plugins/`, он не появится в интерфейсе, и регистрация в `plugin_loader.py` также не удастся. Убедитесь в наличии файла.
