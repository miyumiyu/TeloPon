...
TeloPon은 Google의 최신 AI "Gemini 3.1 Flash Live"를 활용하여 방송자의 목소리와 시청자 댓글을 듣고, **실시간 반응·툭 치는 한마디·요약을 자막(텔롭)으로 표시**하는 차세대 방송 지원 도구입니다.

단순한 "받아쓰기 소프트웨어"가 아닙니다. 방송에 재능 있고 개성 넘치는 **AI 공동 진행자**를 불러들이세요!

![TeloPon 데모](images/demo_ko.gif)
...
### 1. 압도적인 "초저지연"과 "문맥 이해"
Gemini Live API를 직접 활용하여 TeloPon은 "음성 인식 → 텍스트 변환 → AI 전송"이라는 시간이 걸리는 단계를 완전히 생략합니다. 방송자의 말투, 한숨, 심지어 웃음의 뉘앙스까지 즉시 읽어 내어 **대화의 흐름을 끊지 않는 빠른 응답**을 반환합니다.
...
또한 방송자가 침묵하면 AI가 자발적으로 화제를 꺼내는 **"자동 발화"** 와, 일정 시간마다 AI에 내용 요약·정리를 요청하는 **"자동 끼어들기"** 등 자동 개입 모드도 기본 탑재되어 있습니다.

### 6. 실시간 텔롭 이력 확인 "TelopViewer"
AI가 출력한 텔롭을 전용 조작 패널에서 시간순으로 확인할 수 있는 플러그인입니다. 방송 중에 OBS를 확인하지 않아도 텔롭 이력을 앱에서 한눈에 파악할 수 있습니다. B1/B2 태그 색상 구분 표시에도 대응합니다.
...
* **🧠 모델**: 기본적으로 `Auto`로 두세요.
* **💭 사고 레벨**: AI 사고의 깊이를 설정합니다. `minimal`（최고속 응답）〜 `high`（깊은 추론）의 4단계. 방송에서는 기본값 `minimal`이 가장 템포 좋게 작동합니다.
...
* **발화 모드**: 콤보박스에서 자동 개입 방식을 선택합니다.
  * **None**: 자동 개입 없음. AI는 목소리에만 반응합니다.
  * **자동 발화**: 마지막 발화로부터 "개입 시간(초)"이 경과하면 AI가 분위기를 읽고 자발적으로 화제를 꺼내 방송이 조용해지는 것을 방지합니다.
  * **자동 끼어들기**: 마지막 텔롭이 표시된 후 "개입 시간(초)"이 경과하면 AI에 지금까지의 내용 요약·정리를 요청합니다. 정기적인 내용 정리에 최적입니다.
* **개입 시간(초)**: 슬라이더로 자동 개입이 발동하기까지의 대기 시간(초)을 설정합니다. 콤보박스 옆에 **발동까지의 카운트다운**이 표시됩니다 (0이 되면 발동).
...
3. 크기를 16:9 비율 **(권장: 너비 1920 / 높이 1080)** 로 설정하면 완료입니다!

### 🎮 8. OBS에서 텔롭 조작하기
OBS의「상호 작용」버튼을 누르면 텔롭을 직접 **드래그 이동**·**휠 확대축소**·**더블클릭 리셋**·**길게 눌러 삭제**할 수 있습니다. 위치와 스케일은 OBS 재시작 후에도 기억됩니다.

👉 **[OBS 텔롭 조작 가이드（이미지 포함）](docs/ko/05_obs_control.md)**
...
* 📺 **Twitch Live** (`TwitchPlugin.py`)  
  Twitch 채널명 또는 URL을 입력하기만 하면 AI가 시청자의 채팅 댓글을 실시간으로 받아 반응합니다. OAuth2 인증을 하면 댓글 작성, 설문 생성, 예측(베팅), 클립 생성, 방송 제목 변경, 시청자 수 확인 등의 작업도 AI가 자동으로 수행합니다. **인증 없이(IRC 모드)도 채팅 읽기는 동작합니다.**  
  👉 [상세 사용 가이드](docs/ko/plugins/TwitchPlugin.md)

* 📺 **니코니코 생방송** (`NiconicoLivePlugin.py`)  
  니코니코 생방송 댓글·기프트·니코니 광고·앙케이트·통계를 실시간 수집하여 AI에 주입합니다. 로그인 시 앙케이트 생성·운영 댓글 게시·시청자 수 조회도 가능합니다. **로그인 없이도 댓글 읽기는 동작합니다.**  
  👉 [상세 사용 가이드](docs/ko/plugins/NiconicoLivePlugin.md)

* 🎮 **OBS 화면 캡처** (`obs_capture.py`)
...
* 📺 **텔롭 뷰어** (`TelopViewer.py`)
  AI가 출력한 텔롭 이력을 조작 패널에서 실시간으로 확인할 수 있습니다. B1/B2 태그 색상 구분 표시에 대응. OBS 표시 딜레이도 드롭다운으로 설정 가능. 확장 기능 패널의「조작 패널」버튼에서 열기만 하면 사용 가능합니다.

### 🌟 공식 확장 플러그인 (개별 다운로드)

메인 앱을 가볍고 간단하게 유지하기 위해 확장 플러그인은 별도 리포지토리에서 관리됩니다.

🔗 **[TeloPon Extensions (확장 플러그인 목록)](https://github.com/miyumiyu/TeloPon-Extensions)**

| 플러그인 | 설명 |
|---|---|
| 📺 **YouTube Live+** | YouTube OAuth. 댓글 읽기/쓰기, 설문, 제목 변경 |
| 🐦 **X (Twitter)** | 해시태그 수집, 텍스트/썸네일/캡처 트윗 (유료 API) |
| 🔊 **텔롭 읽기 (Windows)** | Windows 내장 음성으로 읽기 |
| 🔊 **텔롭 읽기 (VOICEVOX)** | VOICEVOX 음성 합성 |
| 📊 **PowerPoint 조작** | 음성으로 슬라이드쇼 조작 |
| 💬 **Discord** | Discord 채널 실시간 댓글 |
| 💬 **Slack** | Slack 채널 실시간 댓글 |
| 🎮 **VCI 텔롭 전송** | VirtualCast VCI로 텔롭 전송 |

*(💡 플러그인 매니저에서 다운로드하거나 `.py` 파일을 `plugins` 폴더에 배치하세요)*
...
* 🎮 **[OBS 텔롭 조작 가이드](docs/ko/05_obs_control.md)**: OBS에서 텔롭을 드래그 이동·확대축소·삭제하는 방법.
