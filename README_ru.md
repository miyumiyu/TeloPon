...
### 6. История телопов в реальном времени: «TelopViewer»
Плагин отображает историю телопов, сгенерированных ИИ, в хронологическом порядке в специальной панели управления. Не нужно проверять OBS во время трансляции — история телопов всегда под рукой. Поддерживает цветовую маркировку тегов B1/B2.
...
* **💭 Уровень мышления**: Устанавливает глубину рассуждений ИИ. 4 уровня от `minimal` (максимально быстрый ответ) до `high` (глубокое рассуждение). Для стриминга значение по умолчанию `minimal` обеспечивает лучший темп.
...
3. Установите размер в соотношении 16:9 **(рекомендуется: ширина 1920 / высота 1080)** — и всё готово!

### 🎮 8. Управление телопами в OBS
Нажмите кнопку «Взаимодействие» в OBS, чтобы **перетаскивать**, **масштабировать колёсиком**, **сбрасывать двойным кликом** и **удалять долгим нажатием** телопы. Позиции и масштаб сохраняются после перезапуска OBS.

👉 **[Руководство по управлению телопами в OBS (с иллюстрациями)](docs/ru/05_obs_control.md)**
...
* 📺 **Twitch Live** (`TwitchPlugin.py`)  
  Просто введите имя канала или URL Twitch, чтобы ИИ подхватывал и реагировал на комментарии зрителей в реальном времени. С OAuth2-аутентификацией ИИ может также писать комментарии, создавать опросы, предсказания (ставки), клипы, менять заголовок и получать число зрителей. **Чтение чата работает без аутентификации (IRC-режим).**  
  👉 [Подробное руководство по использованию](docs/ru/plugins/TwitchPlugin.md)

* 📺 **Niconico Live** (`NiconicoLivePlugin.py`)  
  Получение комментариев, подарков, рекламы Niconi, опросов и статистики Niconico Live в реальном времени. При авторизации доступны создание опросов, комментарии оператора и число зрителей. **Чтение комментариев работает без авторизации.**  
  👉 [Подробное руководство](docs/ru/plugins/NiconicoLivePlugin.md)
...
* 💉 **Ввод подсказок / изображений** (`ManualInjector.py`)
...
* 💬 **Чтение комментариев** (`CommentGenerator_read.py`)
...
* 📺 **Просмотр телопов** (`TelopViewer.py`)
  Просмотр истории телопов ИИ в реальном времени через панель управления. Поддерживает цветовую маркировку тегов B1/B2. Задержку отображения в OBS можно настроить через выпадающий список. Просто откройте через кнопку «Панель управления» в панели расширений.
...
| Плагин | Описание |
|---|---|
| 📺 **YouTube Live+** | YouTube OAuth. Чтение/запись, опросы, заголовок |
| 🐦 **X (Twitter)** | Хэштеги, твиты с текстом/миниатюрой/скриншотом (платно) |
| 🔊 **Озвучка телопов (Windows)** | Встроенный голос Windows |
| 🔊 **Озвучка телопов (VOICEVOX)** | VOICEVOX |
| 📊 **Управление PowerPoint** | Управление слайдами голосом |
| 💬 **Discord** | Комментарии из Discord |
| 💬 **Slack** | Комментарии из Slack |
| 🎮 **VCI телоп** | Телопы в VirtualCast VCI |

*(💡 Скачайте через Менеджер плагинов или поместите файлы `.py` в папку `plugins`)*
...
* 🎮 **[Руководство по управлению телопами в OBS](docs/ru/05_obs_control.md)**: Как перетаскивать, масштабировать и удалять телопы в OBS.
