...
Благодаря прямому вызову нативного аудио API Gemini, TeloPon полностью устраняет трудоёмкие этапы «распознавания речи → преобразования в текст → отправки в ИИ». Он мгновенно улавливает нюансы слов стримера, вздохи и даже смех, возвращая **молниеносные ответы, не нарушающие ход разговора**.
...
Кроме того, встроена функция **«Auto Talk (предотвращение тишины)»**: когда стример замолкает, ИИ читает атмосферу и проактивно предлагает тему, чтобы не допустить «мёртвого эфира».
...
* **Auto Talk включён**: Установите этот флажок и задайте «время вмешательства (секунды)» — **когда вы молчите столько секунд, ИИ читает атмосферу и проактивно предлагает тему, чтобы предотвратить «мёртвый эфир»**.
...
| `-at`<br>`--auto_turn` | `TeloPon.exe -at 10` | **Режим автоматических ходов**. Блокирует микрофон, когда речь заканчивается, и разблокирует после ответа ИИ. Укажите тайм-аут в секундах (по умолчанию: 10). `0` = ждать бесконечно. |
| `-seg`<br>`--segment` | `TeloPon.exe -seg` | **Режим периодических сегментов**. Использует таймер Auto Talk из интерфейса как цикл для периодической отправки запросов на резюмирование/тему ИИ. |
